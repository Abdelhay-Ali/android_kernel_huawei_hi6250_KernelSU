			    ...
			    unsigned long flag)
{
	...
	memset(xfer, 0 , sizeof(struct i2c_msg) * 2);
	if (test_flag(I2C_ACCESS_NO_CACHE, &flag)) {
		w_cache = buf;
		w_cache_pa = buf;
		r_cache = val;
...
static int atmel_gpio_request(struct mxt_data *data)
{
	int error = 0;
	TS_LOG_INFO("%s enter\n", __func__);
	if(NULL == data ||NULL == (data->atmel_chip_data)) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		error = -EINVAL;
		goto err;
	}

	if (PMU_GPIO_TYPE == data->atmel_chip_data->vci_gpio_type) {
		error = gpio_request(data->atmel_chip_data->vci_gpio_ctrl, "atmel_vci_gpio_ctrl");
		if (error) {
			TS_LOG_ERR("unable to request vci_gpio_ctrl!\n");
			goto err;
		}
		TS_LOG_INFO("vci_gpio_ctrl request succ\n");
	}
	if (PMU_GPIO_TYPE ==  data->atmel_chip_data->vddio_gpio_type) {
		error = gpio_request(data->atmel_chip_data->vddio_gpio_ctrl, "atmel_vddio_gpio_ctrl");
		if (error) {
			TS_LOG_ERR("unable to request vddio_gpio_ctrl!\n");
			goto ts_vddio_out;
		}
		TS_LOG_INFO("vddio_gpio_ctrl request succ\n");
	}
	return 0;

ts_vddio_out:
	if (PMU_GPIO_TYPE == data->atmel_chip_data->vci_gpio_type)
		gpio_free(data->atmel_chip_data->vci_gpio_ctrl);
err:
	return error;
}

static void atmel_gpio_free(struct mxt_data *data)
{
	TS_LOG_INFO("%s enter\n", __func__);
	if(NULL == data ||NULL == (data->atmel_chip_data)) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
	...
	if (PMU_GPIO_TYPE == data->atmel_chip_data->vci_gpio_type) {
		if(0 <= data->atmel_chip_data->vci_gpio_ctrl)
			gpio_free(data->atmel_chip_data->vci_gpio_ctrl);
	}
	if (PMU_GPIO_TYPE == data->atmel_chip_data->vddio_gpio_type) {
		if (0 <= data->atmel_chip_data->vddio_gpio_ctrl)
			gpio_free(data->atmel_chip_data->vddio_gpio_ctrl);
	}
err:
	return;
}

static int atmel_regulator_set(struct mxt_data *data)
{
	int error = 0;
	if(NULL == data || data->atmel_chip_data == NULL) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		error = -EINVAL;
		goto err;
	}
	if(PMU_LDO_TYPE == data->atmel_chip_data->vci_regulator_type) {
		if (!IS_ERR(data->reg_avdd)) {
			error = regulator_set_voltage(data->reg_avdd,
					  data->atmel_chip_data->regulator_ctr.vci_value,
					  data->atmel_chip_data->regulator_ctr.vci_value);
			if (error < 0) {
			...
			TS_LOG_INFO("%s, reg_avdd set %d(V) success\n", __func__, data->atmel_chip_data->regulator_ctr.vci_value);
		}
	}
	if(PMU_LDO_TYPE == data->atmel_chip_data->vddio_regulator_type) {
		if (!IS_ERR(data->reg_vddio)) {
			error = regulator_set_voltage(data->reg_vddio,
						  data->atmel_chip_data->regulator_ctr.vddio_value,
						  data->atmel_chip_data->regulator_ctr.vddio_value);
			if (error < 0) {
				TS_LOG_ERR
			...
			TS_LOG_INFO("%s, reg_vddio set %d(V) success\n", __func__, data->atmel_chip_data->regulator_ctr.vddio_value);
		}
	}
err:
	return error;
...
static int atmel_power_on(struct mxt_data *data)
{
	...
	gpio_direction_output(data->atmel_chip_data->ts_platform_data->reset_gpio, 0);//parade will set rst gpio input, set rst gpio output at here.
	if(PMU_GPIO_TYPE == data->atmel_chip_data->vddio_gpio_type) {
		TS_LOG_INFO("%s vdda switch gpio on\n", __func__);
		gpio_direction_output(data->atmel_chip_data->vddio_gpio_ctrl, 1);
		...
		if (!IS_ERR(data->reg_vddio)) {
			...
			TS_LOG_INFO("reg_vddio enable is success\n");
		}
	}

	mdelay(1);//sequence request
	if(PMU_GPIO_TYPE == data->atmel_chip_data->vci_gpio_type) {
		TS_LOG_INFO("%s vdda switch gpio on\n", __func__);
		gpio_direction_output(data->atmel_chip_data->vci_gpio_ctrl, 1);
		...
		if (!IS_ERR(data->reg_avdd)) {
			...
			if (error < 0) {
				...
				goto enable_avdd_err;
			}
			TS_LOG_INFO("reg_avdd enable is success\n");
		}
	}
	mdelay(10);//sequence request

	gpio_direction_output(data->atmel_chip_data->ts_platform_data->reset_gpio, 1);
	gpio_direction_input(data->atmel_chip_data->ts_platform_data->irq_gpio);
...
static int atmel_regulator_put(struct mxt_data *data)
{
	int error = 0;
	if(NULL == data || data->atmel_chip_data == NULL) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
	...
	if(PMU_LDO_TYPE == data->atmel_chip_data->vci_regulator_type) {
		if (!IS_ERR(data->reg_avdd)) {
			...
			TS_LOG_INFO("%s, reg_avdd put success\n", __func__);
		}
	}
...
static int atmel_regulator_get(struct mxt_data *data)
{
	int error = 0;
	if(NULL == data || data->atmel_chip_data == NULL) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
	...
	if(PMU_LDO_TYPE == data->atmel_chip_data->vddio_regulator_type) {
		data->reg_vddio = regulator_get(&data->atmel_dev->dev, "atmel-vddio");	/*1.8V*/
		if (IS_ERR(data->reg_vddio)) {
			error = PTR_ERR(data->reg_vddio);
			TS_LOG_ERR("Error %d getting atmel-vdd regulator\n", error);
			goto err;
		}
		TS_LOG_INFO("%s, reg_vddio get success\n", __func__);
	}
	if(PMU_LDO_TYPE == data->atmel_chip_data->vci_regulator_type) {
		data->reg_avdd = regulator_get(&data->atmel_dev->dev, "atmel-avdd");	/*3.1V*/
		if (IS_ERR(data->reg_avdd)) {
			error = PTR_ERR(data->reg_avdd);
			TS_LOG_ERR("Error %d getting atmel-avdd regulator\n", error);
			goto fail;
		}
		TS_LOG_INFO("%s, reg_avdd get success\n", __func__);
	}
	return 0;
...
static void atmel_free_regulators(struct mxt_data *data)
{
	if(!data) {
		TS_LOG_ERR("%s, param invalid\b", __func__);
		return;
	}
	atmel_gpio_free(data);
	atmel_regulator_put(data);
	return;
}

static int atmel_probe_regulators(struct mxt_data *data)
{
	int error = 0;
	 ...
	 * voltage */
	error = atmel_regulator_get(data);
	if (error < 0) {
	...
	error = atmel_regulator_set(data);
	if (error < 0) {
		TS_LOG_ERR("%s,failed to atmel regulator set\n",
			__func__);
		goto pinctrl_get_err;
	}
...
int atmel_probe_info_block(struct mxt_data *data)
{
	...
	return __atmel_read_reg(data, 0, 1, &val);
}
...
int atmel_send_bootloader_cmd(struct mxt_data *data, bool unlock)
{
	...
	ret = atmel_bootloader_write(data, buf, 2);
	if (ret) {
...
static int atmel_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	error = atmel_probe_regulators(data);
	if (error) {
...
static void mxt_parse_module_dts(struct ts_kit_device_data *chip_data)
{
	...
	if (producer != NULL) {
		...
		strncpy(mxt_core_data->module_name, producer, MAX_MODULE_NAME_LEN - 1);
		strncpy(chip_data->module_name, producer, MAX_STR_LEN - 1);
	}
	TS_LOG_INFO("%s, chip_data->module_name = %s\n", __func__, chip_data->module_name);
	retval = of_property_read_string(device, "description", &description);
	if (description != NULL) {
		TS_LOG_INFO("description = %s\n", description);
		strncpy(mxt_core_data->description, description,
			MAX_DESCRIPTION_LEN);
	}
	...
	if (!retval) {
		...
		chip_data->ts_platform_data->feature_info.roi_info.roi_supported = 0;//not support
	}
	...
	if (!retval) {
		...
		chip_data->support_3d_func = 0;// not support
	...
	if (!retval) {
		...
		chip_data->ts_platform_data->feature_info.charger_info.charger_supported = 0;//not support
...
static int atmel_parse_dts(struct device_node *device, struct ts_kit_device_data *chip_data)
{
	...
	error =of_property_read_u32(device, "doze_mode_enable",&chip_data->touch_switch_flag);
	if (error) {
		chip_data->touch_switch_flag  = 0;//not support doze mode
		TS_LOG_ERR("Not support doze mode\n");
	}
	TS_LOG_INFO("touch_switch_flag = %d.\n", chip_data->touch_switch_flag);

	error =of_property_read_u32(device, "ic_type",&chip_data->ic_type);
	if (error) {
	...
	error = of_property_read_u32(device, "vddio_gpio_type", &chip_data->vddio_gpio_type);
	if(error)
	{
		chip_data->vddio_gpio_type = 0;//vddio not use gpio
		TS_LOG_ERR("vddio not use gpio\n");
	}
	TS_LOG_INFO("get vddio_gpio_type = %d\n", chip_data->vddio_gpio_type);
	if(PMU_GPIO_TYPE == chip_data->vddio_gpio_type) {
		chip_data->vddio_gpio_ctrl = of_get_named_gpio(device, "vddio_gpio_enable", 0);
	...
	error = of_property_read_u32(device, "vddio_regulator_type", &chip_data->vddio_regulator_type);
	if(error)
	{
		chip_data->vddio_regulator_type = 0;//vddio not use ldo
		TS_LOG_ERR("vddio not use ldo\n");
	}
	TS_LOG_INFO("get vddio_regulator_type = %d\n", chip_data->vddio_regulator_type);
	if(PMU_LDO_TYPE == chip_data->vddio_regulator_type) {
	...
	error = of_property_read_u32(device, "vci_gpio_type", &chip_data->vci_gpio_type);
	if(error)
	{
		chip_data->vci_gpio_type = 0;//vci not use gpio
		TS_LOG_ERR("vci not use gpio\n");
	}
	TS_LOG_INFO("get vci_gpio_type = %d\n", chip_data->vci_gpio_type);
	if(PMU_GPIO_TYPE == chip_data->vci_gpio_type) {
	...
	error = of_property_read_u32(device, "vci_regulator_type", &chip_data->vci_regulator_type);
	if(error)
	{
		chip_data->vci_regulator_type= 0;//vci not use ldo
		TS_LOG_ERR("vci not use ldo\n");
	}
	TS_LOG_INFO("get vci_regulator_type = %d\n", chip_data->vci_regulator_type);
	if(PMU_LDO_TYPE == chip_data->vci_regulator_type) {
	...
	error = of_property_read_string(device, "chip_name", (const char**)&tmp_buff);
    if (error)
    {
	...
	if(tmp_buff == NULL)
		goto err;
	strncpy(chip_data->chip_name,tmp_buff, MAX_STR_LEN);
	TS_LOG_INFO("%s, chip_name %s\n",__func__ , chip_data->chip_name);

	error = of_property_read_u32(device, "rawdata_get_timeout", &chip_data->rawdata_get_timeout);
	if (!error) {
	...
	if (msg[1] & MXT_T72_NOISE_SUPPRESSION_STATECHG) {
		...
		if ((mxt_core_data->noise_state == MXT_T72_NOISE_SUPPRESSION_VNOIS)
			...
			TS_LOG_DEBUG
			    ...
			    mxt_core_data->noise_state, msg[2] & 0x7);
			if (mxt_core_data->mxt_cur_finger_number == 0) {
