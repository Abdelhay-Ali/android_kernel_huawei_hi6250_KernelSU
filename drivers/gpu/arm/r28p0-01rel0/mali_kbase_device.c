...
static int kbase_device_as_init(struct kbase_device *kbdev, int i)
{
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8316))
		snprintf(poke_name, sizeof(poke_name), poke_format, i);/* unsafe_function_ignore: snprintf */

	snprintf(name, sizeof(name), format, i);/* unsafe_function_ignore: snprintf */
...
static void kbasep_trace_format_msg(struct kbase_trace *trace_msg, char *buffer, int len)
{
	...
	written += MAX(snprintf(buffer + written, MAX(len - written, 0), "%d.%.6d,%d,%d,%s,%pK,", (int)trace_msg->timestamp.tv_sec, (int)(trace_msg->timestamp.tv_nsec / 1000), trace_msg->thread_id, trace_msg->cpu, kbasep_trace_code_string[trace_msg->code], trace_msg->ctx), 0);/* unsafe_function_ignore: snprintf */

	if (trace_msg->katom)
		written += MAX(snprintf(buffer + written, MAX(len - written, 0), "atom %d (ud: 0x%llx 0x%llx)", trace_msg->atom_number, trace_msg->atom_udata[0], trace_msg->atom_udata[1]), 0);/* unsafe_function_ignore: snprintf */

	written += MAX(snprintf(buffer + written, MAX(len - written, 0), ",%.8llx,", trace_msg->gpu_addr), 0);/* unsafe_function_ignore: snprintf */

	/* NOTE: Could add function callbacks to handle different message types */
	/* Jobslot present */
	if (trace_msg->flags & KBASE_TRACE_FLAG_JOBSLOT)
		written += MAX(snprintf(buffer + written, MAX(len - written, 0), "%d", trace_msg->jobslot), 0);/* unsafe_function_ignore: snprintf */

	written += MAX(snprintf(buffer + written, MAX(len - written, 0), ","), 0);/* unsafe_function_ignore: snprintf */

	/* Refcount present */
	if (trace_msg->flags & KBASE_TRACE_FLAG_REFCOUNT)
		written += MAX(snprintf(buffer + written, MAX(len - written, 0), "%d", trace_msg->refcount), 0);/* unsafe_function_ignore: snprintf */

	written += MAX(snprintf(buffer + written, MAX(len - written, 0), ","), 0);/* unsafe_function_ignore: snprintf */

	/* Rest of message */
	written += MAX(snprintf(buffer + written, MAX(len - written, 0), "0x%.8lx", trace_msg->info_val), 0);/* unsafe_function_ignore: snprintf */
}
