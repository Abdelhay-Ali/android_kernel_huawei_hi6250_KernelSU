		...
		off_t offset, size_t size, enum kbase_sync_type sync_fn)
{
	...
	if (likely(cpu_pa == gpu_pa)) {
		...
		memcpy(dst, src, size);/* unsafe_function_ignore: memcpy */
		kunmap(gpu_page);
		kunmap(cpu_page);
		if (sync_fn == KBASE_SYNC_TO_DEVICE)
		...
		struct kbase_va_region *reg)
{
	...
	/* get header_page_number from user space, and set it to insert_pages*/
	header_page_number = reg->gpu_alloc->header_page_number ? reg->gpu_alloc->header_page_number : count;
	err = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn,
			kbase_get_gpu_phy_pages(reg),
			count,
