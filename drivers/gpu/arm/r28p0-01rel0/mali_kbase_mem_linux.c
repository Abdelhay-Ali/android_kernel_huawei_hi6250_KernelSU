	...
	*gpu_va = 0; /* return 0 on failure */
	...
	if (*flags & BASE_MEM_SAME_VA) {
		...
		*gpu_va = (u64) cpu_addr;
		...
		if (kbase_gpu_mmap(kctx, reg, 0, va_pages, 1, true) != 0) {
		    ...
		    u64 *num_pages)
{
	...
	*num_pages = nents * stride;
	...
	if (1) {
#endif
		if (kbase_gpu_mmap(kctx, reg, 0, *num_pages, 1, false) != 0) {
		...
		u64 *flags, u64 header_page_number)
{
	...
	if (*flags & (BASE_MEM_SAME_VA | BASE_MEM_NEED_MMAP)) {
		...
		*gpu_va <<= PAGE_SHIFT;
		...
		if (kbase_gpu_mmap(kctx, reg, 0, *va_pages, 1, false) != 0)
			goto no_gpu_va;
		/* return real GPU VA */
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
	} else {
...
static int kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	kbase_get_compressed_alloc(map->alloc, rel_pgoff,map->alloc->nents - rel_pgoff);
		...
		struct kbase_vmap_struct *map)
{
	...
	kbase_get_compressed_region(reg, (offset_bytes >> PAGE_SHIFT), page_count);
