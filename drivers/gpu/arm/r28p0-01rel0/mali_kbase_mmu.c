	...
	struct kbase_va_region *region)
{
	...
	if (!err) {
		/* insert gpu mappings */
		err = kbase_mmu_insert_pages(kbdev, &kctx->mmu, region->start_pfn + fault_block_pfn,
			&region->cpu_alloc->pages[fault_block_pfn], map_block_size, region->flags, kctx->as_nr ,kbase_reg_current_backed_size(region));
	}
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
...
#if MALI_GMC
			if(phys->is_scrambled)
				flags |= ENTRY_SCRAMBLE_BIT;
#endif
			...
			for (i = 0; i < count; i++) {
				...
				else
				{
					...
					{
						kbdev->mmu_mode->entry_set_ate_scramble_bit(target,
								phys[i], flags, cur_level);
					}
				}
