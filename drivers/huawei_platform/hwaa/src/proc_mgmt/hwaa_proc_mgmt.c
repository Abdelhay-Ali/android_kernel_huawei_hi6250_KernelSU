 ...
 */
s32 hwaa_proc_mgmt_pre_execve(struct task_struct *tsk, const s8 *exe_path)
{
	s32 exe_index;
	...
	// authenticate exe path
	exe_index = hwaa_trusted_zygote_lookup(exe_path);
	if (exe_index < 0)
		return -EINVAL;
	// authenticate parent
	if (hwaa_trusted_zygote_check_parent(parent->pid, parent->comm)) {
		hwaa_pr_err("hwaa trusted zygote check parent fail %d",
			parent->pid);
		return -EINVAL;
	}
	return 0;
