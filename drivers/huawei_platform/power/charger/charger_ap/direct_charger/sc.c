...
static struct dc_volt_para_info_group sc_orig_volt_para[DC_VOLT_GROUP_MAX];
ATOMIC_NOTIFIER_HEAD(direct_charge_sc_fault_notifier_list);


static int direct_charge_sc_set_enable_charger(unsigned int val)
{
	...
	if(ret) {
		hwlog_err("sc: set direct charge disable flags failed\n");
	}
	hwlog_info("sc: set enable_charger = %d\n", di->sysfs_enable_charger);

	return 0;
}

static int direct_charge_sc_get_enable_charger(unsigned int *val)
{
	struct direct_charge_device *di = g_di;
...
static struct power_if_ops sc_power_if_ops = {
	.set_enable_charger = direct_charge_sc_set_enable_charger,
	.get_enable_charger = direct_charge_sc_get_enable_charger,
	.type_name = "sc",
};
...
void direct_charge_sc_check(void)
{
	...
	{
		di->direct_charge_succ_flag = DIRECT_CHARGE_ERROR_ANTI_FAKE_ADAPTOR;
		scp_set_stage_status(SCP_STAGE_DEFAULT);
		di->error_cnt += 1;
		hwlog_err("do_adpator_antifake_check fail!\n");
		return;
	}
...
static void direct_charge_fault_work(struct work_struct *work)
{
	...
	case DIRECT_CHARGE_FAULT_IBUS_OCP:
		...
		break;
	case DIRECT_CHARGE_FAULT_CONV_OCP:
...
static int direct_charge_sc_probe(struct platform_device	*pdev)
{
	...
	di->dev = &pdev->dev;
	np = di->dev->of_node;
	di->orig_volt_para_p = sc_orig_volt_para;
