...
void xengine_report_fallback(struct sock *meta_sk)
{
	struct mptcp_hw_ext_fallback report;
	(void)memset(&report, 0, sizeof(report));

	if (!meta_sk || !meta_sk->sk_socket) {
		mptcp_info("%s: meta_sk or sk_socket is null\n", __func__);
		return;
	}
	...
	report.sock.uid = (int32_t)meta_sk->sk_uid.val;
	report.sock.pid = meta_sk->sk_socket->pid;
	report.sock.fd = meta_sk->sk_socket->fd;
	mptcp_info("%s: uid:%d pid:%d fd:%d mptcp sock\n", __func__,
		report.sock.uid, report.sock.pid, report.sock.fd);

	Emcom_Xengine_MptcpFallback(&report, sizeof(report));
}
#endif
				    ...
				    char *if_name)
{
	...
	case MPTCP_HW_CONF_VALUE_PROXY_INFO: {
		...
		dstval->proxy_info.flag = srcval->proxy_info.flag;
		mptcp_info("%s: proxy_ip:%s proxy_port:%u flag %02X\n", __func__,
			anonymousIPv4addr(srcval->proxy_info.proxy_ip, ip_str, INET_ADDRSTRLEN),
			dstval->proxy_info.proxy_port, srcval->proxy_info.flag);
		break;
	}
...
void mptcp_hw_ext_mod_subflow_prio(struct sock *meta_sk)
{
				       ...
				       struct sockaddr_in *daddr, int addr_len)
{
	...
	if (flags & MPTCP_HW_CONF_VALUE_PROXY_INFO && check_ip_addrss_public_available((struct sockaddr *)daddr)) {
		...
		tp->syn_fallback = !(value.proxy_info.flag & MPTCP_PROXY_FLAG_SYN_FALLBACK);
	}
}
...
int mptcp_proxy_fallback(struct sock *sk, int reason, bool is_add)
{
...
#ifdef CONFIG_HUAWEI_XENGINE
	if (reason == MPTCP_FALLBACK_PROXY_NOMPTCP)
		xengine_report_fallback(sk);
#endif
	...
	if (reason == MPTCP_FALLBACK_PROXY_SYN_TIMEOUT &&
	    !tp->syn_fallback) {
		mptcp_debug("%s: syn_fallback is not set\n", __func__);
		return MPTCP_FALLBACK_UNDO;
	}
