...
int hw_sensor_power_up(sensor_t *s_ctrl)
{
    ...
    for (index = 0; index < power_setting_array->size; index++) {
        ...
        case SENSOR_MIPI_SW:
            ...
            break;
        case SENSOR_MIPI_EN:
            cam_debug("%s, seq_type:%u SENSOR_MIPI_EN", __func__, power_setting->seq_type);
            rc = hw_sensor_gpio_config(MIPI_EN, s_ctrl->board_info,
                power_setting);
            break;
        case SENSOR_RST3:
    ...
    sensor_t *sensor)
{
    ...
    const char *gpio_ctrl_types[IO_MAX] =
        {"reset", "fsin", "pwdn", "vcm_pwdn", "suspend", "suspend2", "reset2", "ldo_en", "ois", "ois2","","","","", "mipisw", "reset3", "pwdn2", "avdd1_en","", "mipi_en"};
    ...
    rc = of_property_read_u32(dev_node, "huawei,pd_valid",
        (unsigned int*)&sensor_info->power_conf.pd_valid);
    ...
    rc = of_property_read_u32(dev_node, "huawei,reset_valid",
        (unsigned int*)&sensor_info->power_conf.reset_valid);
    ...
    rc = of_property_read_u32(dev_node, "huawei,vcmpd_valid",
        (unsigned int*)&sensor_info->power_conf.vcmpd_valid);
    ...
    count = of_property_count_elems_of_size(dev_node, "huawei,csi_index",
       (int)sizeof(u32));
    if (count > 0) {
        of_property_read_u32_array(dev_node, "huawei,csi_index",
            (u32 *)&sensor_info->csi_id,(u32)count);
    } else {
    ...
    count = of_property_count_elems_of_size(dev_node, "huawei,i2c_index",
       (int)sizeof(u32));
    if (count > 0) {
        of_property_read_u32_array(dev_node, "huawei,i2c_index",
           (u32 *)&sensor_info->i2c_id, (u32)count);
    } else {
    ...
    for (i = 0; i < (u32)sensor_info->ldo_num; i++) {
        rc = of_property_read_string_index(dev_node, "huawei,ldo-names",
    ...
    rc = of_property_read_u32(dev_node, "huawei,dvdd_type",
