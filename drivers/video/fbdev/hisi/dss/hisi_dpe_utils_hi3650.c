...
static  uint32_t sbl_al_calib_lut[33] = {
	0,768,2048,3072,4091,5120,6140,7183,8192,
...
void init_igm_gmp_xcc_gm(struct hisi_fb_data_type *hisifd)
{
	...
	if (pinfo->gmp_support == 1) {
		...
		if (1 == pinfo->default_gmp_off) {
			g_gmp_State = 0;
			HISI_FB_DEBUG("For tablet, gmp state disable default!");
		} else {
			//enable gmp
			set_reg(dpp_base + LCP_GMP_BYPASS_EN, 0x0, 1, 0);
			g_gmp_State = 1;
			HISI_FB_DEBUG("For phohe, gmp state enable default!");
		}
...
void dpe_set_gmp_state(struct hisi_fb_data_type *hisifd)
{
	...
	pinfo = &(hisifd->panel_info);
	if (pinfo->gmp_support != 1) {
		HISI_FB_DEBUG("fb%d, not support gmp!\n", hisifd->index);
		return;
	}
