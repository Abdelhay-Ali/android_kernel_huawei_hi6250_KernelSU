...
static void mipi_phy_mask_ctrl(struct hisi_fb_data_type *hisifd, char __iomem *mipi_dsi_base, bool mask, uint8_t txoff_rxulps_en)
{
	if (!txoff_rxulps_en) {
		if (mask) {
...
void mipi_init(struct hisi_fb_data_type *hisifd, char __iomem *mipi_dsi_base)
{
	...
	if ((g_dss_version_tag & FB_ACCEL_DSSV320) && (g_fpga_flag == 0)) {
		mipi_phy_mask_ctrl(hisifd, mipi_dsi_base, true, 0);
...
int mipi_dsi_on(struct platform_device *pdev)
{
	...
	if (hisifd->panel_info.mipi.txoff_rxulps_en) {
		...
		mdelay(1);
		mipi_phy_mask_ctrl(hisifd, hisifd->mipi_dsi0_base, false, 0);
