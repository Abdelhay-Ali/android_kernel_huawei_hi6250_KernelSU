...
struct hwaa_proc_callbacks_t {
	s32 (*pre_execve) (struct task_struct *tsk, const s8 *exe_path);
 ...
 * Description:
 * Input: struct task_struct *tsk: the task executing
 *        const s8 * exe_apth: path name of the file to
 ...
 *         non-zero otherwise
 */
s32 hwaa_proc_pre_execve(struct task_struct *tsk, const s8 *exe_path);
