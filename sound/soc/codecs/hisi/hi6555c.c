...
#define HS_4_POLE_MAX_VOLTAGE                (2650)
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (ret != 0) {
			loge("regulator enable fail, ret=%d\n", ret);
			return ret;
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (ret != 0) {
			loge("regulator disable fail, ret=%d\n", ret);
			return ret;
		}
...
void hi6555c_lineout_rm_recheck_work_func(struct work_struct *work)
{
	...
	WARN_ON(!priv);/*lint !e730*/
	WARN_ON(!priv->codec);/*lint !e730*/
...
static void hi6555c_hs_gpio_init(struct snd_soc_codec *codec)
{
	...
	if(codec == NULL) {
		logd("codec is null pointer\n");
		return;
	}
	priv = snd_soc_codec_get_drvdata(codec);
	node = codec->dev->of_node;
	if(node == NULL||priv == NULL) {
		logd("priv or node is null pointer\n");
		return;
	}
...
#ifdef CONFIG_SND_SOC_HICODEC_DEBUG
	ret = hicodec_debug_init(codec, &hi6555c_dump_info);
	if (ret) {
